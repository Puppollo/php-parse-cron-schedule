 ...
 /* Public methods. */
	...
	public function shouldRun($cronExpression, $testTimestamp = null) {
	 ...
	 // Set up an array with the time we are checking against...
			...
			$cronFireTime = array('parseMinute' => array(intval(date('i', $testTimestamp)), $expressionElements[0]),
			                      'parseHour' => array(intval(date('G', $testTimestamp)), $expressionElements[1]),
			                      'parseDoM' => array(intval(date('j', $testTimestamp)), $expressionElements[2]),
			                      'parseMonth' => array(intval(date('n', $testTimestamp)), $expressionElements[3]),
			                      'parseDoW' => array(intval(date('w', $testTimestamp)), $expressionElements[4]),
			                      'parseYear' => array(intval(date('Y', $testTimestamp)), (isset($expressionElements[5]) ? $expressionElements[5] : date('Y', $testTimestamp))));
 ...
 /* Private and protected methods. */
	...
	protected function _parseStandardCharacters($characterString, $lowerLimit, $upperLimit) {
	 ...
	 // Or it might be a comma separated list of numbers which we can explode...
					} else if (strpos($characterString, ',')) {
	 ...
	 // Finally, if this is a range it's quite easy to deal with...
						} else if (preg_match('/\d{1,2}-\d{1,2}/', $characterString)) {
							$characterChunks = explode('-', $characterString);
